...
export default function RecommendationsPage() {
  ...
  useEffect(() => {// Load user data from userProfile (already available from useAuthRedirect)
    const loadUserData = async () => {
      if (userProfile) {
        try {// Use userProfile directly from the hook
          const dbProfile = userProfile;const userData = {
          ...
          if (dbProfile.mgScore && dbProfile.fsScore) {setScores({
            ...
            if (dbProfile.filiere && trackData[dbProfile.filiere] && dbProfile.grades) {
              ...
              const scoreLevel = getScoreLevel(fs);setScores({ mg, fs, scoreLevel });
            }
          }
        } catch (error) {
    ...
    const loadAllData = async () => {if (isReady && !isRedirecting && userProfile) {
        setIsLoading(true);await loadUserData();await loadUserFavorites();setIsLoading(false);
      } else if (isReady && !isRedirecting) {setIsLoading(false);
      }
    };
  ...
  const filteredRecommendations = useMemo(() => {if (!userData) {return [];
    }

    const userBacType = getBacTypeFromFiliere(userData.filiere);
    const userScore = scores.fs || 0;const filtered = finaleData.filter(item => {
