...
export default function GuidePage() {
  ...
  const { t, loading: translationLoading } = useTranslation(['common', 'guide', 'errors']);
  ...
  const handleFavoriteToggle = async (orientationCode) => {
    if (!isSignedIn) {
      alert(t('favorites.loginRequired', 'guide'));
      return;
    }
      ...
      <div className="min-h-screen bg-[#0f172a] text-gray-100 flex items-center justify-center">
        <div className="text-center">
          ...
          <div className="space-y-3">
            <h2 className="text-2xl font-bold text-white">{t('loading', 'guide')}</h2>
            <p className="text-gray-400">{t('loadingDescription', 'guide')}</p>
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between">
            ...
            <div className="mt-4 sm:mt-0 flex space-x-2">
              ...
              >
                ...
                {t('filters.filterLabel', 'guide')} ({Object.values(filters).filter(v => v && v !== 'latest_score').length})
              </Button>
              <Button
                variant="outline"
              ...
              >
                ...
                {t('filters.favoritesLabel', 'guide')} ({favorites.length})
              </Button>
            </div>
          </div>
        </div>
          ...
          <div className="max-w-7xl mx-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
              ...
              <div className="relative xl:col-span-2">
                ...
                <input
                  type="text"
                  placeholder={t('search.placeholder', 'guide')}
              ...
              <select
                ...
                className="px-4 py-2 bg-slate-700 border border-slate-600 rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-cyan-500"
              >
                <option value="">{t('filters.allPrograms', 'guide')}</option>
                <option value="yes">{t('filters.sevenPercentOnly', 'guide')}</option>
                <option value="no">{t('filters.regularOnly', 'guide')}</option>
              </select>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-4 mt-4">
              ...
              <select
                ...
                className="px-4 py-2 bg-slate-700 border border-slate-600 rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-cyan-500"
              >
                <option value="">{t('filters.allFieldsOfStudy', 'guide')} ({filterOptions.fieldOfStudy.length})</option>
              ...
              <select
                ...
                className="px-4 py-2 bg-slate-700 border border-slate-600 rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-cyan-500"
              >
                <option value="">{t('filters.allUniversities', 'guide')} ({filterOptions.universities.length})</option>
              ...
              <select
                ...
                className="px-4 py-2 bg-slate-700 border border-slate-600 rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-cyan-500"
              >
                <option value="">{t('filters.allBacTypes', 'guide')} ({filterOptions.bacTypes.length})</option>
              ...
              <select
                ...
                className="px-4 py-2 bg-slate-700 border border-slate-600 rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-cyan-500"
              >
                <option value="">{t('filters.allLocations', 'guide')} ({filterOptions.locations.length})</option>
              ...
              <select
                ...
                className="px-4 py-2 bg-slate-700 border border-slate-600 rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-cyan-500"
              >
                <option value="">{t('filters.allInstitutions', 'guide')} ({filterOptions.institutions.length})</option>
  ...
  return (
    <div className="text-center py-12">
      ...
      <h3 className="text-xl font-semibold text-white mb-2">
        {hasActiveFilters ? t('search.noResults', 'guide') : t('search.noPrograms', 'guide')}
      </h3>
      <p className="text-gray-400 mb-6">
        {hasActiveFilters 
          ? t('search.tryChangeFilters', 'guide')
          : t('search.noProgramsFound', 'guide')
        }
      </p>
      {hasActiveFilters && (
        <Button
